...
class Multi_BTS_UNet(nn.Module):
    """
    ...
    def __init__(self, sequences, regions, width, deep_supervision):
        super(Multi_BTS_UNet, self).__init__()
        ...
        self.process_bottleneck2 = ConvInNormLeReLU(widths[2], widths[3])
        self.process_features_map = ConvInNormLeReLU(widths[3] * 3, widths[3])

        # # Define classification decoder layers
        self.classifier = nn.Sequential(
            # nn.AdaptiveAvgPool2d(1),  # Global average pooling
            nn.Flatten(),  # Flatten output
            nn.Linear(widths[3] * 16 * 16, 256),  # Fully-connected layer
            nn.ReLU(),
            # nn.Linear(768, 64),  # Fully-connected layer
            # nn.ReLU(),
            nn.Linear(256, 1)  # Classification output
        )
    ...
    def forward(self, x):
        ...
        # classifier model
        features_extracted = torch.cat([e4, bottleneck, self.process_bottleneck2(bottleneck2)], dim=1)
        features_extracted_post = self.process_features_map(features_extracted)
        predicted_class = self.classifier(features_extracted_post)
        ...
        if self.deep_supervision:
            ...
            return predicted_class, [output3, output2, output1]
        else:
            output1 = self.output1(d1)

            return predicted_class, output1
...
if __name__ == "__main__":
    ...
    model = Multi_BTS_UNet(sequences=1, regions=1, width=6, deep_supervision=True)
    pred_class, segs = model(seq_input)
    ...
    if model.deep_supervision:
        ...
        # print(preds.shape)
        assert seq_ouput.shape == segs.shape
